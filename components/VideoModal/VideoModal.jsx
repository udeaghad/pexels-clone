  ...
  return (
    <div className=" h-[100vh] fixed top-0 bottom-0 left-0 right-0 overflow-y-scroll z-50 w-full bg-[#000000de] flex-col sm:flex-row" style={{display: open ? "flex" : "none"}}>
      ...
      <div className="hidden justify-between items-center w-full lg:flex mb-5">
        ...
        <div>
          <ResponsiveMasonry
            columnsCountBreakPoints={{300: 1, 900: 2}}
          >
            <Masonry >              
                    ...
                    {item.showIcons && (
                      <div className="hidden sm:block">
                        ...
                        <div className="absolute bottom-5 left-5 flex justify-center items-center gap-1 cursor-pointer">
                          <div className={`w-14 h-14 rounded-full border`} style={{backgroundColor: item ? item.avg_color : "gray"}}>
                            <Image src={item.image} alt={item.user.name} width={100} height={100} className="w-full h-full object-cover rounded-full"/>
                          </div>
    
                          <h5 className="text-lg text-slate-100 font-medium whitespace-nowrap">{item.user.name}</h5>
