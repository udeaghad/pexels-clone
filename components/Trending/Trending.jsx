...
const Trending = ({ text }) => {
  const [opt, setOpt] = useState([
    { item: "Trending", selected: true },
    { item: "new", selected: false },
  ]);
    ...
    <div className="mx-4 relative mt-8">
      <div className="flex flex-col justify-start items-start px-5 w-full absolute -z-10">
        <div className="flex justify-between items-center w-full bg-white">
          ...
          <div
            className="h-12 border rounded-md flex justify-center items-center py-1 px-3 text-base text-gray-500 gap-2 bg-white"
        ...
        <div
          className="hidden flex-col border rounded-md p-3 w-32 self-end mt-1 bg-white"
          ref={trendingOptions}
        ...
        >
          {opt.map((item, i) => (
            <div key={i} className="flex flex-col gap-5">
              <div
                className="text-base font-medium cursor-pointer hover:text-gray-700 flex justify-between items-center gap-3"
                style={{ color: item.selected ? "black" : "GrayText" }}
